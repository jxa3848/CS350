...
class CWMachine(StateMachine):
    ...
    def on_enter_dot(self):
        ...
        self.redLight.blink(.5,0,1,False) # this will tell the red led to flash for the time set
    ...
    def on_enter_dash(self):
        ...
        self.blueLight.blink(1.5,0,1,False) # sets the blue light to blink for 1500ms
    ...
    def on_exit_dash(self):
        ...
        self.blueLight.off() # turns the blue light off 
    ...
    def on_enter_dotDashPause(self):
        # wait for 250ms
        sleep(0.25) # the sleep command will cause the system to wait for the time set
    ...
    def on_enter_letterPause(self):
        # wait for 750ms
        sleep(0.75)# the sleep command will cause the system to wait for the time set
    ...
    def on_enter_wordPause(self):
        # wait for 3000ms
        sleep(3)# the sleep command will cause the system to wait for the time set
    ...
    def toggleMessage(self):
        if(self.activeMessage == self.message1): # starts an if this statement where message 1 is the default and if the message 1 is active it will change it to message to
            self.activeMessage = self.message2
        else:
            self.activeMessage = self.message1 # the else statment changes the message back to one if its anything else then message1
        ...
        while not self.endTransmission:
            ...
            for word in wordList:
                ...
                for char in word:
                    ...
                    for x in morse:
                        ...
                        if(x=='.'):
                            ...
                            self.send("doDash")
                            self.send("doDash")
                            # we do these 2 times so we can set them to an off state as we toggle these which is why we send 2 times
                        ...
                        if(morseCounter < lenMorse):
                            self.send("doDDP")
                            self.send("doDDP") # we toggle these which is why we send 2 times and checks if the next thing is a symbols
                    ...
                    if(wordCounter < lenWord):
                        self.send("doLP")
                        self.send("doLP")# we toggle these which is why we send 2 times and checks if the next thing is a letter
                ...
                if(wordsCounter <= lenWords):
                    self.send("doWP")
                    self.send("doWP")# we toggle these which is why we send 2 times and checks if there are more words in the sentence
...
greenButton.when_pressed = cwMachine.processButton # tells the system what to do when the green button is hit
